...
function initMap() {
  ...
  map.on('moveend', debounce(loadStops, 300));
}
...
async function loadStops() {
  ...
  const data = await api(`/api/stops?bbox=${bbox}`);
...
async function selectStop(stopId) {
  ...
  const data = await api(`/api/stops/${stopId}/times`);
...
async function showStopDetail(data, stopId) {
  ...
  const stopData = await api(`/api/stops/${stopId}`);
  ...
  content.innerHTML = `
    <div class="detail-header">
      <h2>${escHtml(stopData.stop_name)}</h2>
    </div>
    <div class="detail-info">
      <div><span class="label">Código</span><br>${escHtml(stopData.stop_code || '—')}</div>
...
document.getElementById('search-input').addEventListener('input', function () {
  ...
  searchTimeout = setTimeout(async () => {
    ...
    if (state.searchQuery !== q) return; // stale

    const container = document.getElementById('search-results');
    container.innerHTML = '';

    // Also filter routes in sidebar
    renderRoutes(routesCache || []);
